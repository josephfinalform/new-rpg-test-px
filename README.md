...
- **Prestige (beyond 100)**: XP earned past max level fills a prestige bar (`get_prestige_threshold()`, base 1000, growing ×1.5 per rank up to `prestige_max_threshold`). Each full bar grants a Prestige point with permanent `+HP/+ATK/+SPD` bonuses (configurable in `level_config.tres`), a `★` suffix on the HUD rank/level label and a PRESTIGE banner effect. Prestige bonuses also benefit from the armor/gear XP multiplier.
